...
|FontEmbedding|String|Specifies if the fonts used in the report should be embedded in the output PDF document. Valid values are: __Full__ , __Subset__ , __None__ . Specifying the __Subset__ option will embed only font glyphs that are used, thus reducing output file size while maintaining portability. Default value: __Subset__ .|
