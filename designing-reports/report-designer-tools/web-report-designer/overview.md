...
* __Assets Manager__ - enables users to organize resources in the Resource Storage of the server. Files and folders can be created, moved, renamed, and deleted, and files can also be uploaded and downloaded. It is accessible through the main menu. It can be also invoked from a property editor to select the source for a PictureBox, external stylesheets, CSV and JSON data source, and reports for Subreport items and ReportBooks (Combined Reports). The contents of the Assets Manager are served from the back-end's [IDefinitionStorage](/api/Telerik.WebReportDesigner.Services.IDefinitionStorage) implementation. The default one is file-based, but can be substituted with a custom implementation that uses virtually any way of storing data in a structured manner. 
